...
MODULE Chem_Sub
   ...
   INTEGER                            , PARAMETER, PUBLIC :: iwrtd = 16            ! Write derivatives in columns for plotting
   ...
   REAL (KIND=dp), DIMENSION (:), ALLOCATABLE, PUBLIC :: abdot               ! Species abundances derivatives
...
!---------------------------------------------------------------------
   ...
   fichi = TRIM(ADJUSTL(outdir)) // TRIM(ADJUSTL(rootnm)) // ".deriv"
   OPEN  (iwrtd, FILE = fichi, STATUS='unknown')
...
n_rxn: DO
   ...
   WRITE (iwrtd,6000) (speci(i), i = 1, nspec) &
                    , "bthb", "ynn", "bnn", "bee", "bchim", "gamh2" &
                    , "gamgr", "gampeg", "zlambd", "xlamoh", "xlamco" &
                    , "xlah2o", "xlamc", "xlamcp", "xlamo", "xlambn" &
                    , "wthb", "xlambe", "gamrc"
...
!---------------------------------------------------------------------
   ...
   DO i = 1, neqt
      ...
      ELSE
         ...
         !-------------------------------------------------------------------------
         ! (101...) exceptions (ncpart)    (units may depend on reaction)
         !-------------------------------------------------------------------------
         !--------------------------------------------------------------------------
         ! (101) H2 + N+ -> NH+ + H    (cm3 s-1)
         !--------------------------------------------------------------------------
         ...
         !--------------------------------------------------------------------------
         ! (105) Reactions avec formule IONPOL1 (voir KIDA)    (cm3 s-1)
         ! Example: C+   + NH3
         !          H2+  + CN
         !          NH+  + CN
         !          HCN+ + H2O
         ...
         ELSE IF (itype(i) == 105) THEN
            !rate = gamm(i) * (ttry/300.0_dp)**alpha(i) * EXP(-ttry/bet(i)) ! pour EXP()T/DE) ala Black H-
            rate = gamm(i) * alpha(i) * (0.62_dp + (0.4767_dp * bet(i) / sqrt(tempr/300.0_dp)))

         !-------------------------------------------------------------------------
         ! (13) Adsorption on grains and neutralization [CHIMES]
         !-------------------------------------------------------------------------
         ELSE IF (itype(i) == 13) THEN
            rate = gamm(i) * xpi * radgr * radgr * SQRT(8.0_dp * xk * tempr / xmol(ir1) / xpi)

         !-------------------------------------------------------------------------
         ! (17) photodesorption from grains (nphdes)    (s-1)
         !-------------------------------------------------------------------------
         ...
         !-------------------------------------------------------------------------
         ! STOP the code is itype is not recognized
         !-------------------------------------------------------------------------
         ELSE
            PRINT *, "Not recognized reaction type : ", itype(i)
            STOP
         ENDIF
